...
# NOTE :
# session.timeout.ms is commented in client.properties because this code is a producer
# timout make sense for consumers only
...
# -----------------------------------------------------------------------------
k_Topic = "topic_1"
k_Key = "fraud_detection_2"  # key for writing in topic_1
...
def fetch_and_store(producer) -> None:
    ...
    while True:
        try:
            ...
            producer.produce(k_Topic, key=k_Key, value=json.dumps(data).encode("utf-8"), on_delivery=on_produce)
            producer.flush()  # Otherwise is never called
