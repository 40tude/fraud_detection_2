...
In the [fraud_detection_2 project](https://github.com/40tude/fraud_detection_2), the ``Consume Data`` block, the yellow rectangle of the initial specifications, is positioned between the "real-time" data producer and the MLflow block.
...
<p align="center">
<img src="./assets/flux_meter_2_topics.png" alt="drawing" width="800"/>
<p>

While reading the remaining of this document, we should keep in mind the previous figure (from initial specifications) knowing that our target architecture is the one below :  
...
1. On the right hand side of the original figure, simulated transactions from the ``Real-time Data producer`` are dumped at ``speed_1`` into ``topic_1``. 
    ...
    * Once again, just because the app have lost the connection with the Real-time Data producer doesn't mean everything stops. The system does the best it can. 
    * This point is **important**: whatever happens, the application must do its utmost to consume the transactions received, so that it can make predictions and sound the alarm if necessary.
...
<!-- ###################################################################### -->
## Note
The above shows that the model receives requests and issues predictions at ``speed_2``. This is to simplify the explanation and to emphasize that the prediction speed is different from either the transaction acquisition speed or the database storage speed. 
...
* Another case could arise if, instead of reading from `topic_1` one record at a time, the consumer feeding the model reads several records at once. There are several possible scenarios we could envision : 
    * The consumer asks to read half of the available records in `topic_1`.
    * The consumer asks to read 4 records at a time, because it "knows" that predictions will be made on 4 models running on parallel processes. 
    * etc.
...
<p>
 ...
 1. Transactions received from the Real-time Data producer block.
 1. Information provided to the model to get prediction in return.
...
* On the right hand side of the figure above, there's no guarantee that the format of transactions remains stable over time. Nor is it certain that today's format corresponds exactly to the format expected by the model to make predictions. A transaction could, for example, include more features than necessary. So we have 2 options:
...
* The first one doesn't have a title (I put the word `empty` below) it's just an index
...
A quick test (see `99_tooling\02_API_test\quick_test.ipynb`) shows that the simulated transactions received from the Real-time Data producer block have the following format:
...
```
One could make a quick test on this [page](https://real-time-payments-api.herokuapp.com/docs#/default/current_transactions_current_transactions_get) 

<p align="center">
<img src="./assets/img062.png" alt="drawing" width="800"/>
<p>


In table form, we can see that :
* There are 21 columns. The ones missing are : 
    * the index
...
We're interested in the topic in which a producer will deposit simulated transactions that he has retrieved from the "Real-time Data producer".

<p align="center">
<img src="./assets/img065.png" alt="drawing" width="800"/>
<p>
...
* This is a Python code that retrieves simulated transactions from the "Real-time Data producer" and deposits them in ``topic_1``. 
...
* The script begins by running the `secrets.ps1` script.
...
<!-- ###################################################################### -->
# What's next ?
* Go to the directory `consumer` and read the [README.md](../02_consumer/README.md) file. 
