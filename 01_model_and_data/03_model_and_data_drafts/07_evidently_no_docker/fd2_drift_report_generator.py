...
k_Local_Reports_Dir = k_Current_dir / "reports"
k_Local_Data_Dir = k_Current_dir / "../../04_data"
...
def bias_production_dataset(production_df: pd.DataFrame) -> pd.DataFrame:
    ...
    production_drifted_df.to_csv(Path(k_Local_Data_Dir) / k_Production_Drifted_Dataset, index=False)
...
def fetch_datasets() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        df = pd.read_csv(fraud_test_file, nrows=k_Number_of_samples)
    except FileNotFoundError as e:
        g_logger.error(f"File '{fraud_test_file}' not found. Ensure the file path is correct.")
        raise FileNotFoundError(f"File '{fraud_test_file}' not found.") from e
    ...
    reference_sample_file = Path(k_Local_Data_Dir) / k_Reference_Dataset_Dataset
    production_sample_file = Path(k_Local_Data_Dir) / k_Production_Dataset_Dataset
...
def generate_report(reference_df: pd.DataFrame, production_df: pd.DataFrame) -> None:
    ...
    output_file = k_Local_Reports_Dir / f"data_drift_report_{timestamp1}.html"
...
if __name__ == "__main__":
    ...
    k_Local_Reports_Dir.mkdir(parents=True, exist_ok=True)
    g_logger.info(f"{k_Local_Reports_Dir} created (if not already existing)")
